# Study Planner — Codex Context

This file gives Codex full context about this project so it can make changes accurately without needing re-explanation each session.
...
### Core concept
The app imports **plan.json files** — structured learning plans that Codex (in chat) generates for any topic. Each plan contains modules with hours, tags, and resource links. The app turns these into a dynamic calendar schedule.
...
This is the format Codex (in chat) generates when a user asks for a new study plan. The app imports this file.
...
The planner was built to track this course and has been extended to be a universal tool for any learning plan. The owner uses Codex (chat) to generate `plan.json` files for new topics and imports them into this app.
